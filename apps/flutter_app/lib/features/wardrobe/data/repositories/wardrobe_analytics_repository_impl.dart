...
/// Mock implementation of WardrobeAnalyticsRepository
/// Provides realistic analytics data simulation for development and testing
class WardrobeAnalyticsRepositoryImpl implements WardrobeAnalyticsRepository {
  // Simulated analytics cache
  final Map<String, WardrobeAnalytics> _analyticsCache = {};
  final Map<String, List<Map<String, dynamic>>> _usageHistory = {};
  final Random _random = Random();

  WardrobeAnalyticsRepositoryImpl() {
    _initializeMockData();
  }

  void _initializeMockData() {
    // Initialize some mock usage history for realistic analytics
    final now = DateTime.now();
    final userId = 'user_1';
    
    _usageHistory[userId] = _generateMockUsageHistory(now);
  }

  List<Map<String, dynamic>> _generateMockUsageHistory(DateTime endDate) {
    final history = <Map<String, dynamic>>[];
    final categories = ['Tops', 'Bottoms', 'Dresses', 'Outerwear', 'Shoes', 'Accessories'];
    final occasions = ['Work', 'Casual', 'Formal', 'Sport', 'Party', 'Weekend'];
    
    // Generate 90 days of mock usage data
    for (int i = 0; i < 90; i++) {
      final date = endDate.subtract(Duration(days: i));
      final itemsWorn = _random.nextInt(5) + 3; // 3-7 items per day
      
      for (int j = 0; j < itemsWorn; j++) {
        history.add({
          'date': date,
          'itemId': 'item_${_random.nextInt(100)}',
          'category': categories[_random.nextInt(categories.length)],
          'occasion': occasions[_random.nextInt(occasions.length)],
          'rating': (_random.nextDouble() * 5).round().toDouble(),
          'cost': (_random.nextDouble() * 200) + 20, // $20-220
        });
      }
    }
  ...
  Future<Either<Failure, WardrobeAnalytics>> generateAnalytics({
    ...
    try {
      ...
      final now = DateTime.now();
      final effectiveStartDate = startDate ?? _getStartDateForPeriod(period, now);
      final effectiveEndDate = endDate ?? now;
      
      final analytics = WardrobeAnalytics(
        id: 'analytics_${userId}_${period.name}_${now.millisecondsSinceEpoch}',
        userId: userId,
        period: period,
        startDate: effectiveStartDate,
        endDate: effectiveEndDate,
        usageStats: _generateUsageStats(period),
        categoryStats: _generateCategoryStats(),
        colorAnalysis: _generateColorAnalysis(),
        mostWornOutfits: _generateOutfitFrequencies(true),
        mostWornItems: _generateItemFrequencies(true),
        leastWornItems: _generateItemFrequencies(false),
      ...
      // Cache the generated analytics
      final cacheKey = '${userId}_${period.name}';
      _analyticsCache[cacheKey] = analytics;
      
      return Right(analytics);
    } catch (e) {
      return Left(ServerFailure(message: 'Failed to generate analytics: $e'));
    }
  }

  WardrobeUsageStats _generateUsageStats(AnalyticsPeriod period) {
    final totalItems = 80 + _random.nextInt(40); // 80-120 items
    final wornItems = (totalItems * (0.6 + _random.nextDouble() * 0.3)).round(); // 60-90%
    final unwornItems = totalItems - wornItems;
    final usagePercentage = (wornItems / totalItems * 100);
    ...
    return WardrobeUsageStats(
      ...
      totalOutfits: 30 + _random.nextInt(20),
      uniqueOutfitsWorn: 15 + _random.nextInt(15),
      averageWearPerItem: 2 + _random.nextInt(8),
      costPerWear: 5.0 + (_random.nextDouble() * 20),
      newItemsAdded: _random.nextInt(5),
